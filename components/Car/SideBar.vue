...
const priceRangeText = computed(() => {
  ...
  // Si prix minimum & prix maximal n'existe pas, retourne "Any"
  if(!minPrice && !maxPrice) return "Any"
  // Sinon si prix min n'existe pas et que maxPrice existe, retourne le prix inférieur au prix max
  else if(!minPrice && maxPrice) {
    return `< ${maxPrice}`
  }
  // Sinon si minPrice existe et que maxPrice n'existe pas, retourne le prix supérieur au prix min
  else if(minPrice && !maxPrice) {
    return `> ${minPrice}`
  }
  // Sinon retourne prix minimal et prix maximal
  else {
